...
/*   Updated: 2020/10/30 13:44:55 by scopycat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_xyzpoint		find_plane_point(t_scene *scene, t_index fig)
{
	...
	while (i--)
		tmp_pl = tmp_pl->next;
	return (normalize_orient(tmp_pl->center, tmp_pl->orient, \
		scene->camera->center));
}
...
t_xyzpoint		find_triangle_point(t_scene *scene, t_index fig)
{
	...
	while (i--)
		tmp_tr = tmp_tr->next;
	return (normalize_orient(tmp_tr->angle_one, vector_mult(substruct_vector(\
			tmp_tr->angle_two, tmp_tr->angle_one), substruct_vector(\
			tmp_tr->angle_three, tmp_tr->angle_one)), scene->camera->center));
}
...
t_xyzpoint		find_square_point(t_scene *scene, t_index fig)
{
	...
	while (i--)
		tmp_sq = tmp_sq->next;
	return (normalize_orient(tmp_sq->center, tmp_sq->orient, \
		scene->camera->center));
}
...
t_xyzpoint		find_cylin_point(t_scene *scene, t_index fig, t_xyzpoint canvas)
{
	...
	while (i--)
		tmp_cy = tmp_cy->next;
	if (fig.t == check_cylin_wall(tmp_cy, canvas, MAXFLOAT, \
		scene->camera->center))
		point = tmp_cy->normal;
	else
		point = tmp_cy->orient;
